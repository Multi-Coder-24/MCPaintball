...
public class LightBlueExplosiveStation extends Block
{

    public LightBlueExplosiveStation()
    {
    ...
    public InteractionResult use(BlockState pState, Level pLevel, BlockPos pPos, Player pPlayer, InteractionHand pHand, BlockHitResult pHit)
    {
        if(!pLevel.isClientSide())
        {
            ...
            if(TeamUtils.IsTeam(SP,8))
            {
                ...
                {
                    SP.drop(new ItemStack(iteminit.PAINTBALL_GRENADE.get(),16),true);
                    SP.drop(new ItemStack(blockinit.LIGHT_BLUE_EXPLOSIVE.get(),16),true);
