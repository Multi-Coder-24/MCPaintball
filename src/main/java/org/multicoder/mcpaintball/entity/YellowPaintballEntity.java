...
public class YellowPaintballEntity extends AbstractArrow {
    public YellowPaintballEntity(EntityType<? extends AbstractArrow> type, Level level) {
        super(type, level);
    }

    public YellowPaintballEntity(Player mob, Level level, @Nullable ItemStack firedFromWeapon) {
        super(MCPaintballEntities.YELLOW_PAINTBALL, mob, level, new ItemStack(Items.ARROW), firedFromWeapon);
    }
    ...
    protected void onHitEntity(@NonNull EntityHitResult result) {
        if(result.getEntity() instanceof Player Target){
            ...
            DebugHelper.HandleDebug(result,level(),4,(Player) this.getOwner());
        }
        super.discard();
    }
}
