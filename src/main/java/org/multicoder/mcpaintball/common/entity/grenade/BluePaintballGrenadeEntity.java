...
public class BluePaintballGrenadeEntity extends ThrowableItemProjectile implements ItemSupplier {
    ...
    public BluePaintballGrenadeEntity(EntityType<?> p_37438_, LivingEntity p_37439_, Level p_37440_) {
        ...
        if (MCPaintballWorldData.INSTANCE.MatchStarted)
        {
            ...
            E.getHitPlayers().keySet().forEach(player ->
            {
                if (MCPaintballTeamsDataHelper.HasTeam(player))
                {
                    ...
                    {
                        MCPaintballWorldData.incrementByOrdinal(PaintballTeam.BLUE.ordinal());
                    }
                }
            });
        }
