    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide()) {
            if (!Screen.hasControlDown()) {
                ...
                {
                    try {
                        if (MCPaintballWorldData.INSTANCE.StartedByName(cap.getName(RifleItem.class), RifleItem.class)) {
                            if (SP.getItemInHand(hand).getDamageValue() < 4) {
                                PaintballTeam Team = cap.GetTeam(RifleItem.class);
                                AbstractArrow Paintball = null;
                                try
                                {
                                    Paintball = new PaintballEntity(Team.getPaintball(RifleItem.class), player, level);
                ...
                try {
                    ReloadManager.ReloadWeapon(Weapon, SP, RifleItem.class);
                } catch (AccessException e) {MCPaintball.SECURITY_LOG.fatal(MarkerManager.getMarker("MCPaintball"),"An attempt Was made to access an anti-cheat protected class");}
            }
        }
