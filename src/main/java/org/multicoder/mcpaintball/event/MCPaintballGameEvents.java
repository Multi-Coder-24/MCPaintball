...
public class MCPaintballGameEvents {
    ...
    public static void clientEndTick(Minecraft minecraft) {
        ...
        while (MCPaintballKeybinding.OPEN_ADMIN_SCREEN.consumeClick()){
            if(Objects.requireNonNull(minecraft.player).permissions().hasPermission(Permissions.COMMANDS_MODERATOR)){
                Screen Parent = minecraft.screen;
                minecraft.setScreen(new AdminScreen(Parent));
            }
        }
        while (MCPaintballKeybinding.KIT_GIVE.consumeClick()){
            ClientPlayNetworking.send(new KitSenderC2SPacket());
        }
    }
}
