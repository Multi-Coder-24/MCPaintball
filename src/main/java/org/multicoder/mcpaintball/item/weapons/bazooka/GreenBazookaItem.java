...
public class GreenBazookaItem extends Item
{
        ...
        if(!level.isClientSide)
        {
            player.getCapability(PlayerTeamCapabilityProvider.CAPABILITY).ifPresent(cap ->
            {
                ...
                if (Team == 2)
                {
                    ...
                    level.playSound(null,player.blockPosition(), soundinit.BAZOOKA.get(), SoundSource.PLAYERS,1f,1f);
                }
            });
        }
    ...
    public void appendHoverText(ItemStack pStack, Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced)
    {
        ...
        {
            pTooltipComponents.add(new TranslatableComponent("text.mcpaintball.bazooka_spec").withStyle(ChatFormatting.BOLD));
        }
        else
        {
