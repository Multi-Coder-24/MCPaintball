...
public class SightGrenadeItem extends Item {

    public SightGrenadeItem(Properties properties) {
        super(properties);
    }
    ...
    public @NonNull InteractionResult use(@NonNull Level level, @NonNull Player player, @NonNull InteractionHand hand) {
        if(!level.isClientSide()) {
            if (MCPaintballGameEvents.INSTANCE.tournamentStarted && MCPaintballGameEvents.INSTANCE.roundStarted) {
                ...
                SightGrenadeEntity entity = new SightGrenadeEntity(MCPaintballEntities.SIGHT_GRENADE, player, level, player.getItemInHand(hand));
